...
/** Create the complete Urban Study 05 model. No DOM, renderer or global event handlers. */
export function createBowRiverModel({ traffic = true, mergeStatic = true } = {}) {
const scene = new THREE.Group();
scene.name = 'Bow River - Urban Study 05';
...
if (!traffic) { for (const c of cars) { scene.remove(c.mesh); c.mesh.geometry.dispose(); } cars.length=0; }
if (mergeStatic) {
...
const labelData=[['Bow River',440,180,'river'],['14 Street bridge',657,160,''],['Memorial Drive',327,111,''],['Sunalta station',355,448,'',44],['Greyhound',462,307,'',27],['Cowboys Park',851,347,''],['Armoury',912,316,'',32],['Rail meets street',933,263,'',6],['Contemporary Calgary',883,227,'',34],['Broadview',275,36,'']];
let disposed=false;
function update(deltaSeconds=0) {
 if(disposed)return;
 const dt=Number.isFinite(deltaSeconds)?Math.max(0,deltaSeconds):0;
 for(const c of cars){c.t=(c.t+dt*c.speed)%1;const p=c.curve.getPointAt(c.t),v=c.curve.getTangentAt(c.t);c.mesh.position.copy(p);c.mesh.position.y+=1;c.mesh.position.x+=v.z*(c.laneOffset??3);c.mesh.position.z-=v.x*(c.laneOffset??3);c.mesh.rotation.y=Math.atan2(v.x,v.z);}
}
function dispose(){
 if(disposed)return;disposed=true;
 const geometries=new Set(), ownedMaterials=new Set(Object.values(materials));
 scene.traverse(o=>{if(o.geometry)geometries.add(o.geometry);if(o.material)for(const m of Array.isArray(o.material)?o.material:[o.material])ownedMaterials.add(m);});
 for(const g of geometries)g.dispose();for(const m of ownedMaterials)m.dispose();
 scene.removeFromParent();scene.clear();
}
update(0);
scene.updateMatrixWorld(true);
return {
 group:scene, update, dispose,
 bounds:new THREE.Box3().setFromObject(scene),
 labels:labelData.map(([text,x,z,kind,height])=>({text,kind,position:new THREE.Vector3(x-497,height??(kind?3:22),z-236)})),
 routes:{roads:roadCurves,rail:railCurve,interchange:interchangeCurves,greyhoundBypass,downtownDrive},
 metadata:{version:5,upAxis:'Y',north:'-Z',units:'reference-image units; not surveyed metres',origin:[497,236],sourceCommit:'7f486187fe62665e65bfeb4250c9d17bc9c85a8f'}
};
}

/** Optional lights for the example appearance. Add separately to your host scene. */
export function createBowRiverLighting(){
 const group=new THREE.Group();
 group.add(new THREE.HemisphereLight('#ebf6ff','#77816d',2.3));
 const sun=new THREE.DirectionalLight('#fff5df',3);sun.position.set(-300,600,-350);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);
 Object.assign(sun.shadow.camera,{left:-700,right:700,top:700,bottom:-700,near:1,far:1600});sun.shadow.normalBias=.7;group.add(sun);group.add(sun.target);
 return group;
}

export const cameraPresets={roads:{eye:[-65,200,370],target:[5,0,75]},interchange:{eye:[-45,185,-70],target:[187,3,89]},station:{eye:[-300,115,440],target:[-140,24,212]},park:{eye:[183,162,-95],target:[369,13,60]},rail:{eye:[230,90,-20],target:[383,6,38]}};
